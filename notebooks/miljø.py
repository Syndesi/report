...
def _():
    ...
    from astral.sun import blue_hour, dawn, dusk, golden_hour, sun, twilight
...
def _(
    ...
    def create_solar_chart(solar: SolarTimes, title: str) -> go.Figure:
        ...
        for start, end, color, label in segments:
            ...
            fig.add_trace(go.Bar(
                ...
                hovertemplate=(
                    f"<b>{label}</b><br>"
                    f"{_fmt(start)} – {_fmt(end)}<extra></extra>"
                ),
    ...
    def create_solar_legend(solar: SolarTimes) -> mo.Html:
        tz              = solar.timezone
        midnight_start  = datetime.datetime(
            solar.date.year, solar.date.month, solar.date.day, tzinfo=tz
        )
        ...
        columns = [
            (
                SolarColor.NIGHT, "Night",
                [(None, solar.astronomical_dawn), (solar.astronomical_dusk, None)],
            ),
            (
                SolarColor.ASTRONOMICAL_TWILIGHT, "Astr. Twilight",
                [(solar.astronomical_dawn, solar.nautical_dawn),
                 (solar.nautical_dusk,     solar.astronomical_dusk)],
            ),
            (
                SolarColor.NAUTICAL_TWILIGHT, "Naut. Twilight",
                [(solar.nautical_dawn, solar.civil_dawn),
                 (solar.civil_dusk,    solar.nautical_dusk)],
            ),
            (
                SolarColor.CIVIL_TWILIGHT, "Civil Twilight",
                [(solar.civil_dawn, solar.sunrise),
                 (solar.sunset,     solar.civil_dusk)],
            ),
            (
                SolarColor.DAY, "Day",
        ...
        def _resolve(s, e):
            return (midnight_start if s is None else s,
                    midnight_end   if e is None else e)
        ...
        def _duration_str(intervals) -> str:
            secs = sum(
                int((_resolve(s, e)[1] - _resolve(s, e)[0]).total_seconds())
